...
class JobparserPipeline:
    ...
    def process_item(self, item, spider):
        if spider.name == 'hhru':
            item['salary_min'] = self.process_salary(item['salary'])[0]
            item['salary_max'] = self.process_salary(item['salary'])[1]
            item['currency'] = self.process_salary(item['salary'])[2]
            #Добавил еще в базу тип зарплаты (на руки, до вычета налогов), т. к. в реальной ситуации это важное обстоятельство
            item['salary_type'] = self.process_salary(item['salary'])[3]
            collection = self.mongobase[spider.name]
            collection.insert_one(item)
        else:
            item['salary_min'] = self.process_salary_sj(item['salary'])[0]
            item['salary_max'] = self.process_salary_sj(item['salary'])[1]
            item['currency'] = self.process_salary_sj(item['salary'])[2]
            collection = self.mongobase[spider.name]
            collection.insert_one(item)
        return item
